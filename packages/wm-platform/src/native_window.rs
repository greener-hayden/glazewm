...
impl WindowId {
  ...
  ///
  /// Returns `None` if the element has no resolvable window ID — the lookup
  /// failed, or the ID is zero (a sentinel that is never a valid window).
  /// Such elements must not be tracked: a fabricated `WindowId(0)` would
  /// collide with every other unresolved element, since window identity is
  /// the `WindowId`.
  #[cfg(target_os = "macos")]
  pub(crate) fn from_window_element(
    ...
    (result == AXError::Success && window_id != 0).then_some(Self(window_id))
  }
}
