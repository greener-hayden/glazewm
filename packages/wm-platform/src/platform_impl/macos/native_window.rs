...
pub(crate) struct NativeWindow {
  ...
  /// application swaps the element backing a still-live window (e.g. Finder
  /// on folder navigation). All clones share the `Arc`, so a refresh is seen
  /// by every holder, including the window manager's own copy.
...
impl NativeWindow {
  ...
  /// window. All clones share the same `Arc`, so the new element is seen by
  /// every holder. Must be called on the event loop thread.
