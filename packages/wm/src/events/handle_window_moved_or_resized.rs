...
pub fn handle_window_moved_or_resized(
  ...
  if let Some(window) = found_window {
    ...
    // `MovedOrResized` event, and a window parked for a reveal is still
    // at its old dimensions — so the frame that arrives here can be one
    // that happens to cover the workspace. `should_fullscreen` is
    // inferred from that frame, so a stale reading promotes a tiling
    // window to fullscreen, which then sizes it to the workspace and
    // keeps the inference true. `is_maximized` is a direct OS query, so
    // it stays trusted; only the geometric inference is held back.
    let is_settling = cfg!(target_os = "macos")
      && !matches!(window.display_state(), DisplayState::Shown);

    // Handle a window being maximized or entering fullscreen.
    if is_maximized || (should_fullscreen && !is_settling) {
